...
run_model = function(data) {
  ...
  fit.stan <- run.stan.to.estimate(data, 
                                   ...
                                   nr.iterations = 50000, anchors.input = TRUE,
                                   model = 'poisson')
...
data_output_filtered = run_model(data = data_filtered)
data_output_full = run_model(data = data)

data_output_full %>%
  ggplot(mapping = aes(x = case, y = theta)) +
