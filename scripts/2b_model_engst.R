...
fit_model = function(data, iterations = 40000, model = "bernoulli"){
  ...
  run.stan.to.estimate <- function(data, nr.iterations, model, seed.input,
    ...
    if (model == 'poisson'){
      if (anchors.input != FALSE){
        ...
        model_file <- 'scripts/code_helpers/stan_bernoulli_1D_decenter_anchors.stan'
    ...
    # Select the correct model to estimate the anchors
    if (model == 'poisson'){
      model_file_pre <- 'scripts/code_helpers/stan_poisson_pre_run.stan'
    } else if(model == "bernoulli") {
      model_file_pre <- 'scripts/code_helpers/stan_bernoulli_pre_run.stan'
    }
    ...
    fit2 <- stan(
      ...
      init = init.list,
      seed = seed.input,
      control=list(adapt_delta=0.95) # this can reduce divergence errors
    )
    time2 <- proc.time()
  ...
  fit.stan <- run.stan.to.estimate(data, 
                                   ...
                                   nr.iterations = iterations, nr.cores.input = 8, anchors.input = TRUE,
                                   model = model)
